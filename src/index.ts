...
/**
 * New Caledonia Open Data (data.gouv.nc) — OpenDataSoft MCP.
 *
 * New Caledonia Open Data — open data for New Caledonia (French Pacific territory) via the OpenDataSoft Explore v2.1 API: government, environment, economy & geography. Keyless.
 * Use `search_datasets` (find a dataset_id) -> `dataset_info` (schema) -> `query` (ODSQL records).
 */


const BASE = 'https://data.gouv.nc';
const UA = 'pipeworx-mcp-data-newcaledonia/1.0 (+https://pipeworx.io)';

const tools: McpToolExport['tools'] = [
  {
    name: 'search_datasets',
    description: 'Search New Caledonia Open Data for datasets by keyword (government, environment, economy & geography). Returns dataset_ids (pass to query/dataset_info), titles, themes and record counts.',
  ...
  {
    name: 'dataset_info',
    description: 'Get metadata for one New Caledonia Open Data dataset (fields/schema, themes, record count) — call before query to learn the column names.',
    inputSchema: { type: 'object', properties: {
  ...
  {
    name: 'query',
    description: 'Query records from a New Caledonia Open Data dataset with ODSQL. Filter (where), aggregate (group_by/select), sort (order_by), paginate (limit/offset).',
    inputSchema: { type: 'object', properties: {
      dataset_id: { type: 'string', description: 'Dataset id from search_datasets.' },
...
async function get(path: string): Promise<unknown> {
  ...
  if (!res.ok) throw new Error(`New Caledonia Open Data: ${res.status} ${await res.text().then((t) => t.slice(0, 160))}`);
  return res.json();
}
